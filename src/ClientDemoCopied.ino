...
IPAddress ipserver(192,168,1,6);  // The server's IP address
 ...
 void setup()  
 {
  ...
  { 
     ...
     status = sokClient.wifiBegin(ssid, pword);
      //status = sokClient.connect(chserver);
  }  
  while(sokClient.connect(ipserver, PORTID)!= true) {
    Serial.println( "Connection retry..." );
    delay(1000);
  }
  //if(sokClient.connect(ipserver, PORTID)== trure)
  sokClient.setDataArrivedDelegate(dataArrived);
  sokClient.send("Hello PC I'm Arduino!!");
  //
...
void loop() 
{
  ...
  /*
  if (client.connect(ipserver, PORTID)) 
  {
